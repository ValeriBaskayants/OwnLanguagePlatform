...
function useSpeech() {
  ...
  function play(text: string, rate = 1.0) {
  ...
  return { speaking, paused, play, togglePause, stop };
}

// ─── Controls component ───────────────────────────────────────────────────────
function AudioControls({ text, rate, onRateChange }: {
  text: string;
  rate: number;
  onRateChange: (r: number) => void;
}) {
  const { t } = useTranslation();
  const { speaking, paused, play, togglePause } = useSpeech();

  return (
    <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '1.25rem' }}>
      <button className="btn btn-primary btn-sm" onClick={() => play(text, rate)}>
        🔊 {t('listening.play')}
      </button>
      {speaking && (
      ...
      <div style={{ display: 'flex', gap: '0.35rem', alignItems: 'center' }}>
        ...
        {[0.7, 1.0, 1.3].map(r => (
          <button key={r} onClick={() => onRateChange(r)} style={{
            padding: '0.25rem 0.6rem', borderRadius: '100px', border: '1px solid',
            borderColor: rate === r ? 'var(--accent)' : 'var(--border-color)',
            background: rate === r ? 'var(--accent-light)' : 'transparent',
            color: rate === r ? 'var(--accent)' : 'var(--text-muted)',
...
function DictationExercise({ ex, onDone }: { ex: ListeningExercise; onDone: (score: number, xp: number) => void }) {
  ...
  const submit = async () => {
    ...
    const res = await sessionsService.create({
      ...
      answers: [{ itemId: ex._id, userAnswer: userText, correctAnswer: ex.text, isCorrect: score >= 70, timeSpentMs: Date.now() - startTime.current, topic: ex.topic }],
    ...
    <div className="fade-in">
      ...
      <div style={{ fontSize: '0.875rem', lineHeight: 2, fontFamily: 'DM Mono, monospace', background: 'var(--surface-elevated)', padding: '1rem', borderRadius: '10px' }}>
        {result.correctWords.map((w: string, i: number) => (
          <span key={i} style={{
            marginRight: '4px', padding: '1px 4px', borderRadius: '3px',
            background: result.userWords[i] === w ? 'var(--success-light)' : 'var(--danger-light)',
            color: result.userWords[i] === w ? 'var(--success)' : 'var(--danger)',
          }}>{w}</span>
        ))}
      </div>
  ...
  return (
    <div>
      <AudioControls text={ex.text} rate={rate} onRateChange={setRate} />
      <textarea
        value={userText}
...
function ComprehensionExercise({ ex, onDone }: { ex: ListeningExercise; onDone: (score: number, xp: number) => void }) {
  ...
  const next = async () => {
    if (qIdx + 1 >= (ex.questions?.length || 0)) {
      ...
      const res = await sessionsService.create({ type: 'listening', level: ex.level, answers: sessionAnswers, durationMs: Date.now() - startTime.current });
      ...
      {q && (
        <div>
          <p style={{ fontWeight: 600, marginBottom: '1rem', lineHeight: 1.6 }}>
            <span style={{ color: 'var(--text-muted)', fontSize: '0.82rem', fontFamily: 'DM Mono, monospace' }}>Q{qIdx + 1}/{ex.questions?.length} </span>
            {q.question}
          </p>
              ...
              return (
                <button key={i} onClick={() => handleSelect(i)} disabled={selected !== null}
                  style={{
                    ...
                    cursor: selected !== null ? 'default' : 'pointer', fontSize: '0.9rem', transition: 'all 0.15s',
                  }}>
                  {opt}
                </button>
              );
          ...
          {selected !== null && (
            <button className="btn btn-primary" style={{ marginTop: '0.75rem' }} onClick={next}>
              {qIdx + 1 >= (ex.questions?.length || 0) ? 'See Results' : 'Next →'}
            </button>
          )}
        </div>
      )}
  ...
  return (
    <div className="page">
      <div className="page-header">
        ...
        <p style={{ color: 'var(--text-secondary)' }}>Improve listening with dictation and comprehension</p>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState icon="🎧" title="No listening exercises" description="Add exercises from the Admin panel." />
      ) : (
        <div className="grid-2">
          {exercises.map(ex => (
            <Card key={ex._id} onClick={() => setSelected(ex)} style={{ cursor: 'pointer' }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>{ex.text.substring(0, 90)}…</p>
            </Card>
          ))}
        </div>
      )}
