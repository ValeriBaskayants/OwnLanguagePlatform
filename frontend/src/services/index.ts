...
export const vocabularyService = {
  ...
  getFlashcards: (level?: string, limit = 20) => api.get('/api/vocabulary/flashcards', { params: { level, limit } }).then(r => r.data),
  getUserProgress: () => api.get('/api/vocabulary/user-progress').then(r => r.data),
  review: (wordId: string, quality: 0 | 1 | 2 | 3) => api.post('/api/vocabulary/review', { wordId, quality }).then(r => r.data),
};
...
export const translateWord = async (word: string, targetLang = 'ru'): Promise<string> => {
  try {
